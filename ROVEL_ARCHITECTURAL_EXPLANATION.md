...
flowchart TD
    ...
    subgraph Execution ["Background Processing"]
        ...
        Nginx["🛡️ Nginx Proxy<br>(Routing & SSL)"]
    end

    Redis -->|4. Polls Jobs| Worker
    Worker -->|5. Compiles Image & Spawns| Docker
    Docker -->|6. Spawns| UserApps["📦 User Applications<br>(Isolated Containers)"]
    Worker -->|7. Writes Config & Reloads| Nginx
    Nginx -->|8. Proxy Passes Traffic| UserApps
```
...
### 3. Execution Worker: Node.js & TypeScript
* **What it is**: A persistent background service (daemon) running on the server.
* **Why it was used**: It listens for jobs in the Redis queue. When a job arrives, the worker clones the Git repository, auto-detects the framework, writes a Dockerfile, builds the Docker image, runs the container, and updates Nginx config files. Using TypeScript prevents runtime errors through static type-checking.
...
* **Why it was used**: Rovel needs to track users, projects, environment variables, and deployment logs. A relational database ensures data integrity (e.g., deleting a project automatically deletes its associated deployments). Prisma automatically compiles our TypeScript database queries into optimized SQL.
...
* **Why it was used**: It allows Rovel to host completely different applications (e.g., a Python app and a Next.js app) on the same server without conflicts. Docker ensures that an application cannot access the database or file system of another application. It also lets us set strict limits on CPU and memory consumption.

### 6. Reverse Proxy: Nginx
* **What it is**: A high-performance web server and reverse proxy.
* **Why it was used**: Next.js runs on port 3000, and deployed developer apps run on random ports (e.g., 3001, 3002). Nginx acts as the front gate of the server. It listens on public web ports (80 and 443) and routes traffic to the correct application based on the incoming domain name (e.g., routing `neo-clock.apps.rovel.dev` to `127.0.0.1:3001`).
...
## ⌨️ Part 2: Key Commands Explained (What & Why)
...
```
* **What it does**: Loads environment variables from the configuration file and runs the Prisma schema push command.
* **Why we ran it**: Prisma requires the `DATABASE_URL` variable to connect to PostgreSQL. Simply running `npm run db:push` in the terminal does not load the variables from `.env` or `.env.production`. Using a small Node wrapper ensures that the environment variables are loaded into memory before the database sync runs.
...
## 🎓 Part 3: Presentation Guide for Your Professor
...
* **Event-Driven Architecture**: Explain that the control plane (Next.js) and the build execution layer (Worker) are decoupled. They communicate asynchronously via a Redis queue. This prevents long-running build tasks from locking up the user interface.
* **Process Isolation**: Highlight that user applications are deployed in isolated Docker containers with restricted memory limits (`512MB`) and CPU quotas (`0.5`). This prevents a poorly written application from crashing other apps on the server.
* **Dynamic Routing**: Explain how the reverse proxy works. Nginx dynamically routes HTTP/HTTPS traffic to different internal port allocations on localhost based on the domain headers of the request.
* **Infrastructure Automation**: Demonstrate the CI/CD pipeline. Push a minor change to GitHub and show how GitHub Actions automatically logs into the EC2 instance, runs the build steps, and updates the platform with zero downtime.
...
* **Q: Why did you choose to build a custom worker instead of using a standard CI/CD tool?**
  * *A*: Standard CI/CD tools build code but do not handle dynamic routing, subdomain allocation, or wildcard SSL generation for external developers. Rovel acts as a complete PaaS, managing the entire application lifecycle from code push to active URL generation.
